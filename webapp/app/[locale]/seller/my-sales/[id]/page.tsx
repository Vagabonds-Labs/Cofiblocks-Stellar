...
export default function SaleDetailPage() {
  ...
  const { order, loading, error, refetch } = useSale(orderId)
  ...
  if (error || !order) {
    return (
      <div className="min-h-screen bg-gray-50">
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <button
            onClick={() => router.push(`/${locale}/seller/my-sales`)}
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <button
                onClick={async () => {
                  ...
                  setConfirming(true)
                  try {
                    await sellsService.confirmDelivery(orderId)
                    // Redirect to my-sales page on success
                    router.push(`/${locale}/seller/my-sales`)
                  } catch (err) {
                    console.error('Failed to confirm delivery:', err)
                    alert(err instanceof Error ? err.message : t('my_sales.confirm_delivery_error'))
                    setConfirming(false)
                  }
                }}
                disabled={confirming}
