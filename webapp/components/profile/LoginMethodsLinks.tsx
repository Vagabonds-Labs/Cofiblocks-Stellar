...
export function LoginMethodsLinks({ t }: { t: any }) {
  const { ready, authenticated, user, linkEmail, linkGoogle, linkApple, linkTwitter, linkPasskey } = usePrivy()

  if (!ready || !authenticated || !user) return null

  const passkeys = user.linkedAccounts.filter((account) => account.type === 'passkey').length

  const methods: Array<{ key: string; label: string; detail?: string | null; linked: boolean; link: () => void }> = [
    { key: 'email', label: t('auth.login_method_email'), detail: user.email?.address, linked: !!user.email, link: linkEmail },
    ...(SOCIAL_LOGIN_ENABLED
      ? [
          { key: 'google', label: 'Google', detail: user.google?.email, linked: !!user.google, link: linkGoogle },
          { key: 'apple', label: 'Apple', detail: user.apple?.email, linked: !!user.apple, link: linkApple },
          {
            key: 'twitter',
            label: 'X (Twitter)',
      ...
      ? [
          {
            ...
            detail: passkeys > 0 ? t('auth.login_method_passkey_count', { count: passkeys }) : null,
            linked: passkeys > 0,
            link: () => linkPasskey(),
          },
        ]
  ...
  return (
    <div className="mt-10 border border-gray-200 rounded-lg p-4 text-left">
      <div className="flex items-start gap-3">
        <UserCircleIcon className="w-5 h-5 text-gray-500 mt-0.5" />
        <div className="flex-1">
          <h3 className="font-semibold">{t('auth.login_methods_title')}</h3>
          <p className="text-sm text-gray-600 mt-1">{t('auth.login_methods_description')}</p>

          <ul className="mt-4 divide-y divide-gray-100">
            {methods.map((method) => (
              <li key={method.key} className="flex items-center justify-between gap-3 py-2">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-900">{method.label}</p>
                  {method.detail && <p className="text-xs text-gray-500 truncate">{method.detail}</p>}
                </div>
                {method.linked ? (
                  ...
                  <button
                    onClick={method.link}
                    className="px-3 py-1 border border-gray-300 rounded-md text-xs hover:bg-gray-50"
                  >
