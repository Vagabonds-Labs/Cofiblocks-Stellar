...
import { CreateEventData } from '@/services/api/events/types'
...
export function useCreateEvent(t?: ReturnType<typeof useTranslations>) {
  ...
  // Fetch timezones and set default timezone on mount
  useEffect(() => {
    async function fetchTimezones() {
      try {
        ...
        // Set browser timezone if it exists in the list, otherwise use UTC
        const browserTz = getBrowserTimezone()
        const defaultTimezone = fetchedTimezones.includes(browserTz) 
          ? browserTz 
          : fetchedTimezones[0]
        ...
        setTimezones([''])
        setForm(prev => ({ ...prev, timezone: getBrowserTimezone() }))
      } finally {
  ...
  function validate(): boolean {
    ...
    if (!form.timezone) {
      newErrors.timezone = translations('admin_events.add.error_timezone_required')
    }
    ...
    } else if (form.isAllDay && form.startDate && form.endDate) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const startDateISO = formatDateTime(form.startDate, form.startTime, form.isAllDay)
      const endDateISO = form.isAllDay 
        ? undefined 
